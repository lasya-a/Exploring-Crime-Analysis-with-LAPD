...
def predict():
 ...
 926: 'TRAIN WRECKING'}
        ...
        # Check if the value is not None and is numeric before converting it to a float
        area = float(request.form.get('AREA')) if request.form.get('AREA') is not None and request.form.get('AREA').isnumeric() else 0.0
        rpt_dist = float(request.form.get('Rpt_Dist_No')) if request.form.get('Rpt_Dist_No') is not None and request.form.get('Rpt_Dist_No').isnumeric() else 0.0
